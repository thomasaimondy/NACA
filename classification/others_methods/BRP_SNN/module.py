...
class FA_wrapper(nn.Module):
    ...
    def reset_weights(self):
        ...
        # torch.nn.init.uniform_(self.fixed_fb_weights)
        # self.fixed_fb_weights = torch.ones_like(self.fixed_fb_weights)
...
class TrainingHook(nn.Module):
    def __init__(self, label_features, dim_hook, train_mode):
        ...
        assert train_mode in ["BP", "FA", "DFA", "DRTP", "sDFA", "shallow", "BRP"], "=== ERROR: Unsupported hook training mode " + train_mode + "."
        
        # Feedback weights definition (FA feedback weights are handled in the FA_wrapper class)
        if self.train_mode in ["DFA", "DRTP", "sDFA", "BRP"]:
    ...
    def reset_weights(self):
        torch.nn.init.kaiming_uniform_(self.fixed_fb_weights)
        # self.fixed_fb_weights.data = torch.ones_like(self.fixed_fb_weights.data)
        self.fixed_fb_weights.requires_grad = False

    def forward(self, input, labels, y, input_):
        return trainingHook(input, labels, y, self.fixed_fb_weights, input_, self.train_mode if (self.train_mode != "FA") else "BP") #FA is handled in FA_wrapper, not in TrainingHook
