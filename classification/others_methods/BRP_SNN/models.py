...
class NetworkBuilder(nn.Module):
            ...
            try:
                if layer[0] == "CONV":
                    ...
                    if (i == 0):
                        ...
                        if utils.args.dataset in ['tidigits', 'MNIST']:
    ...
    def forward(self, input, labels):
        ...
        for step in range(self.spike_window):
            ...
            elif utils.args.dataset == 'gesture':
                x = (input > torch.rand(input.size()).float().cuda() * self.randKill).reshape(input.shape[0],-1)
            else:
            ...
            for i in range(len(self.layers)):
                ...
                # if i == 2:
                    # print(self.layers[2].fc.weight.data.cpu().numpy().max(), self.layers[2].fc.weight.data.cpu().numpy().min())
                    # np.save('STDP_weight_B_ones.npy', self.layers[2].fc.weight.data.cpu().numpy())
...
def mem_update(ops, x, mem, spike, drop=None,lateral=None):
    # print('mem')
    # print(mem.shape)
    # print('ops(x)')
    # print(ops(x).shape)
...
class FC_block(nn.Module):
    ...
    def forward(self, x, labels, y):
        ...
        self.time_counter += 1
        if self.dropout != 0:
            self.mem, self.spike = mem_update(self.fc, x, self.mem, self.spike, self.drop)
        else:
        ...
        # x = self.fc(x)
        # x = self.act(x)
        x = self.hook(self.spike, labels, y, self.out)
