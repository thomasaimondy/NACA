...
def main():
    ...
    parser.add_argument('--spike-window', type=int, default=20, help='The time clock for neurons. Default: 20.')#10,20,30
    ...
    parser.add_argument('--output-act', type=str, choices = {'sigmoid', 'tanh', 'none', 'relu'}, default='sigmoid', help='Type of activation for the network output layer - Sigmoid (sigmoid), Tanh (tanh), none (none). Default: sigmoid.')
    ...
    parser.add_argument('--pool', type=str, choices=['Avg', 'Max'], default='Avg', help='(default=%(default)s)')
    parser.add_argument('--quickexit', action='store_true')
    parser.add_argument('--pre-encoding', action='store_true')
    parser.add_argument('--brpscale', type=float, default=1e-4)
    timeclock = time.strftime("%Y%m%d%Hh%Mm%Ss", time.localtime())

    args = parser.parse_args()
    utils.args = args
    args.codename = timeclock + '_' +args.codename+'_'+str(args.seed)
    mkd(args)
